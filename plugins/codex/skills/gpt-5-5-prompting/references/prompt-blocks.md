...
Opt-in blocks for Codex / GPT-5 prompts. On GPT-5.5, apply these selectively: pull in a block only when it changes behavior for the run, and do not stack them by default. Start from the outcome-first skeleton in `SKILL.md`; many runs need no XML block.
...
Use when the prompt feeds a downstream parser or needs a stable wrapper. On GPT-5.5, plain `# Goal` and `# Success criteria` headers usually suffice.
...
Use when you want concise prose instead of a schema. On GPT-5.5, prefer asking for concise output over detailed length rules.
...
Use when the run may take a while. On GPT-5.5, also consider a one- or two-sentence preamble before the first tool call.
