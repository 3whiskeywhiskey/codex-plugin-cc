...
Better: start from a fresh outcome-first skeleton with Goal, Success criteria, Constraints, Output, and Stop rules. Keep only instructions that change behavior.
...
Better: state the outcome and success criteria; let the model choose the path. Script exact steps only when correctness or safety requires that path.

### Overusing absolutes

Bad: `ALWAYS`, `NEVER`, `must`, and `only` sprinkled across judgment calls.

Better: reserve absolutes for true invariants. For judgment calls, write decision rules such as "if X, prefer Y."

### Restating defaults

Bad: including the current date, inline output schemas, or older-model boilerplate.

Better: delete any line that is not changing behavior. Use structured outputs for schemas where available.
...
Raising `reasoning.effort` belongs after the contract is tight, and only when the task actually needs it.
