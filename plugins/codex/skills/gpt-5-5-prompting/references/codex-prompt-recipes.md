...
On GPT-5.5, prefer the outcome-first skeleton from `SKILL.md` and reach for these XML recipes only when the run needs an explicit contract. Copy the smallest recipe that fits, then trim any block that is not changing behavior.
